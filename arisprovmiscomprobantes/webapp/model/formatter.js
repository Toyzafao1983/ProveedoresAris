sap.ui.define([
	...
	return {
		...
		formatFechaODataHora: function (sFecha) {
			if (!sFecha) {
				return "";
			}

			try {
				// Extraer número de milisegundos de /Date(XXXXXXXXXXXX)/
				const timestamp = parseInt(sFecha.replace(/[^0-9]/g, ""), 10);
				const oDate = new Date(timestamp);

				// Formatear a dd/MM/yyyy HH:mm:ss
				const dia = String(oDate.getDate()).padStart(2, "0");
				const mes = String(oDate.getMonth() + 1).padStart(2, "0");
				const anio = oDate.getFullYear();

				const horas = String(oDate.getHours()).padStart(2, "0");
				const minutos = String(oDate.getMinutes()).padStart(2, "0");
				const segundos = String(oDate.getSeconds()).padStart(2, "0");

				return `${dia}/${mes}/${anio} ${horas}:${minutos}:${segundos}`;
			} catch (error) {
				console.error("Error al formatear fecha OData:", error);
				return "";
			}
		},
