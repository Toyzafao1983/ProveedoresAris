sap.ui.define([
	...
	return {
		...
		sharePointDeleteSync: function (url, callback) {
			$.ajax({
				url: url,
				method: "DELETE",
				async: false,
				success: function (result, textStatus, xhr) {
				callback({ iCode: 1, c: "suc", u: url, m: "Exito HTTP - DELETE", data: result, status: xhr.status });
				},
				error: function (xhr, status, error) {
				callback({ iCode: -1, c: "ex", u: url, m: "Error HTTP - DELETE", data: error, status: xhr.status });
				}
			});
		},
	};
