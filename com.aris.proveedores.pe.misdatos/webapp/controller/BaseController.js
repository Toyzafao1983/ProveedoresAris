sap.ui.define([
	...
	'sap/m/Token',
	"com/aris/proveedores/pe/misdatos/services/ServiceOdata",
	...
	var sMessage = "";
	var that;
	return Controller.extend("com.aris.proveedores.pe.misdatos.controller.BaseController", {
		formatter: Formatter,
		local: window.location.href.indexOf('launchpad') == -1 ? true : false,
		localModel: true,
		AdminUser: true,
		userSet: "kestefo@ravaconsulting.com.pe",
		route: "com.aris.proveedores.pe.misdatos",
		_getUsers: function () {
			that = this;
			try {
		...
		_applyProveedorUserProfile: function (oUser) {
			...
			if (oModelUser) {
				...
				// Mantengo /sRol porque el XML actual lo usa.
				// /sRol = true  => usuario interno.
				// /sRol = false => usuario externo.
				oModelUser.setProperty("/sRol", oProfile.bIsInterno);
		...
		fnExportarExcelDetalle: function (aData, sAuthor, sIndicador) {
			if (!aData || aData.length < 1) {
				this.getMessageBox("error", this.getI18nText("errorNoDataExport"));
				return;
			}

			// 🔹 Crear columnas dinámicas según los datos
			var aCols = this.createColumnConfigDetalle(aData);

			var dDate = new Date();
			var sGetTime = dDate.getTime().toString();
			var sTitleExcel = (sIndicador || "Export") + '-' + sGetTime + '.xlsx';

			var sTitleDocument = this.isEmpty(sAuthor)
				? this.getI18nText("Token")
				: this.getI18nText("Token") + "-" + sAuthor;
		...
		createColumnConfig: function () {
			return [
				{
					label: this.getI18nText("ExtxtProveedor"),
					property: 'SupplierName',
					width: '20',
					type: 'String'
				},
				{
					label: this.getI18nText("ExtxtRUC"),
					property: 'TaxID',
					width: '15'
				},
				{
					label: this.getI18nText("ExtxtCodSap"),
					property: 'SupplierCode',
					width: '20',
					type: 'String'
				},
				{
					label: this.getI18nText("ExtxtAdress"),
					property: 'to_Address/results/0/Street',
					width: '15'
				},
				{
					label: this.getI18nText("ExtxtCity"),
					property: 'to_Address/results/0/City',
					width: '15'
				},
				{
					label: this.getI18nText("ExtxtBanks"),
					property: 'to_BankAccounts/results/0/BankName',
					width: '15'
				},
				{
					label: this.getI18nText("ExtxtHowManyBanks"),
					property: 'to_BankAccounts/results/0/BankAccount',
					width: '20',
					type: 'String'
				},
				{
					label: this.getI18nText("ExtxtContactPerson"),
					property: 'to_Contacts/results/0/ContactPerson',
					width: '15'
				},
				{
					label: this.getI18nText("ExtxtMobilePhone"),
					property: 'to_Contacts/results/0/PhoneNumber',
					width: '20',
					type: 'String'
				},
				{
					label: this.getI18nText("ExtxtEmail"),
					property: 'to_Emails/results/0/Email',
					width: '20',
					type: 'String'
				}
		...
		createColumnConfigDetalle: function (aData) {
			// aData es el array de proveedores que vas a exportar
			let iMaxContacts = 0;
			let iMaxEmails = 0;
			let iMaxBanks = 0;

			aData.forEach(item => {
				iMaxContacts = Math.max(iMaxContacts, item.to_Contacts?.results?.length || 0);
				iMaxEmails = Math.max(iMaxEmails, item.to_Emails?.results?.length || 0);
				iMaxBanks = Math.max(iMaxBanks, item.to_BankAccounts?.results?.length || 0);
			});

			let aCols = [
				{ label: this.getI18nText("ExtxtProveedor"), property: 'SupplierName', width: '20', type: 'String' },
				{ label: this.getI18nText("ExtxtRUC"), property: 'TaxID', width: '15' },
				{ label: this.getI18nText("ExtxtCodSap"), property: 'SupplierCode', width: '20', type: 'String' },
				{ label: this.getI18nText("ExtxtAdress"), property: 'to_Address/results/0/Street', width: '15' },
				{ label: this.getI18nText("ExtxtCountry"), property: 'to_Address/results/0/City', width: '15' }
			];

			// 🔹 Bancos dinámicos
			for (let i = 0; i < iMaxBanks; i++) {
				aCols.push({
					label: `${this.getI18nText("ExtxtBanks")} ${i + 1}`,
					property: `to_BankAccounts/results/${i}/BankName`,
					width: '20',
					type: 'String'
				});
				aCols.push({
					label: `${this.getI18nText("ExtxtHowManyBanks")} ${i + 1}`,
					property: `to_BankAccounts/results/${i}/BankAccount`,
					width: '20',
					type: 'String'
				});
			}
			// 🔹 Contactos dinámicos
			for (let i = 0; i < iMaxContacts; i++) {
				aCols.push({
					label: `${this.getI18nText("ExtxtContactPerson")} ${i + 1}`,
					property: `to_Contacts/results/${i}/ContactPerson`,
					width: '20',
					type: 'String'
				});
				aCols.push({
					label: `${this.getI18nText("ExtxtMobilePhone")} ${i + 1}`,
					property: `to_Contacts/results/${i}/PhoneNumber`,
					width: '20',
					type: 'String'
				});
			}
			// 🔹 Emails dinámicos
			for (let i = 0; i < iMaxEmails; i++) {
				aCols.push({
					label: `${this.getI18nText("ExtxtEmail")} ${i + 1}`,
					property: `to_Emails/results/${i}/Email`,
					width: '20',
					type: 'String'
				});
			}
		...
		ChangeMultiImput: function (oEvent) {
			...
			var sCustom = oSource.data("custom");
			var sCustomParameter = sCustom.split("/")[0];
			var sTextParameter = sCustom.split("/")[1];
			var oKey = [];
			var oText = [];
			...
			if (aTokens.length === 0) {
				...
				aTokens.forEach(function (oToken) {
					oKey.push(oToken.getKey());
					oText.push(oToken.getText());
				});
		...
		getPrueba: function (context) {
			...
			try {
				...
				return new Promise(function (resolve, reject) {
					let sUrl = "";
					if (that.local) {
						const sPath = "/sap/opu/odata/sap/ZMMB_PORTALPROVEEDORES/Supplier?&$format=json";
						sUrl = that.getOwnerComponent().getManifestObject().resolveUri(sPath);
					} else {
						const sPath = jQuery.sap.getModulePath(that.route) + "/S4HANA/sap/opu/odata/sap/ZMMB_PORTALPROVEEDORES/Supplier?&$format=json";
						sUrl = sPath;
					}
		...
		//funciones auxiliares para evitar deduplicacion en resultados y traiga el mas completo

		_isFilled: function (v) {
			return v !== null && v !== undefined && String(v).trim() !== "";
		},

		_getNavCount: function (item, navName) {
			const nav = item && item[navName];
			const res = nav && nav.results;
			return Array.isArray(res) ? res.length : 0;
		},

		_scoreSupplier: function (item) {

			let s = 0;
			if (this._isFilled(item.SupplierName)) s += 2;
			if (this._isFilled(item.SupplierCode)) s += 2;
			if (this._isFilled(item.TaxID1) || this._isFilled(item.TaxID2)) s += 2;
			s += this._getNavCount(item, "to_Address") * 3;
			s += this._getNavCount(item, "to_Emails") * 2;
			s += this._getNavCount(item, "to_BankAccounts") * 2;
			s += this._getNavCount(item, "to_Contacts") * 1;
		...
		_applyBankColumnsVisibility: function (aResults) {
			const oModelProyect = this.getModel("oModelProyect");
			const aData = Array.isArray(aResults) ? aResults : [];

			let aBanks = [];
			aData.forEach(function (oSupplier) {
				const aSupplierBanks = (oSupplier?.to_BankAccounts?.results) || [];
				aBanks = aBanks.concat(aSupplierBanks);
			});

			const hasForeignBank = aBanks.some(function (oRow) {
				const sCountry = (oRow.BankCountry || "").trim().toUpperCase();
				return sCountry !== "" && sCountry !== "PE";
			});

			aData.forEach(function (oSupplier) {
				const aSupplierBanks = (oSupplier?.to_BankAccounts?.results) || [];

				aSupplierBanks.forEach(function (oRow) {
					const sCountry = (oRow.BankCountry || "").trim().toUpperCase();

					if (!hasForeignBank || sCountry === "" || sCountry === "PE") {
						oRow.BankKey = "";
						oRow.IBAN = "";
					}
				});
			...
			if (oModelProyect) {
				oModelProyect.setProperty("/ui/hasBankKeyColumn", hasForeignBank);
				oModelProyect.setProperty("/ui/hasIBANColumn", hasForeignBank);
