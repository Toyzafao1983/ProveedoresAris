sap.ui.define([
	...
	return {
		...
		serviceRootpath: function () {
			var rootPath;
				rootPath = "/RavaService/IPROVIDER_FASE_2";
		...
		ERPGetAsync: function (url, callback) {
			$.ajax({
				...
				dataType: 'json',
				success: function (result) {
					if( !Array.isArray(result.d.results) ){delete result.d.results.__metadata}
					var respuestaService = {
						iCode:1,
						c: "suc",
						u: url,
						m: "Exito HTTP - GET",
						data: Array.isArray(result.d.results) ? result.d.results.map(({ __metadata, ...rest }) => rest) : result.d.results     
					};
					return callback(respuestaService);
				},
				error: function (xhr, status, error) {
		...
		ERPGetSyncNotResult: function (url, callback) {
			$.ajax({
				url: url,
				method: "GET",
				async: false,
				...
				dataType: 'json',
				success: function (result) {
					if( !Array.isArray(result.d) ){delete result.d.__metadata}
					var respuestaService = {
						iCode:1,
						c: "suc",
						u: url,
						m: "Exito HTTP - GET",
						data: Array.isArray(result.d) ? result.d.map(({ __metadata, ...rest }) => rest) : result.d
					};
					return callback(respuestaService);
				},
				error: function (xhr, status, error) {
...
ERPPostTokenAsync: async function (urlget, urlpost, data, callback, extraHeaders = {}) {
    return new Promise((resolve, reject) => {
        $.ajax({
            ...
            success: function (oData, status, response) {
                ...
                $.ajax({
                    ...
                    headers: Object.assign(
                        ...
                        extraHeaders // 🔧 permite inyectar "X-HTTP-Method: MERGE"
                    ),
                    data: JSON.stringify(data),
                    success: function (result) {
                        resolve({ iCode: 1, m: "Éxito HTTP POST", data: result?.d });
                    },
                    error: function (xhr, status, error) {
                        reject({
                            iCode: -1,
            ...
            error: function (xhr, status, error) {
                reject({
                    iCode: -1,
                    m: "Error HTTP GET al obtener token",
                    status: xhr.status,
                    responseText: xhr.responseText
                });
            }
...
ERPPostTokenAsync2: async function (urlget, urlpost, data, callback, extraHeaders = {}) {
    return new Promise((resolve, reject) => {
        // 1) GET para obtener X-CSRF-Token
        $.ajax({
            url: urlget,
            type: "GET",
            async: true,
            ...
            success: function (oData, status, response) {
                ...
                // 2) POST real con el token y los headers extra
                $.ajax({
                    url: urlpost,
                    method: "POST",
                    async: true,
                    headers: Object.assign(
                        ...
                        extraHeaders // 👈 aquí entran gbtnte-*, X-HTTP-Method, etc
                    ),
                    data: JSON.stringify(data || {}),
                    success: function (result) {
		...
		ERPMergeTokenAsync: async function (urlget, urlmerge, data, callback) {
			await $.ajax({
				url: urlget,
				type: "GET",
				async: true,
				headers: { "x-CSRF-Token": "Fetch" },
				success: async function (oData, status, response) {
					var token = response.getResponseHeader("x-csrf-token");
					await $.ajax({
						url: urlmerge,
						method: "MERGE",
		...
		sharePointUploadProgressSync: function (url, file, onProgress, callback) {
			...
			xhr.onload = function () {
				...
				callback({ iCode: 1, data: jsonResponse });
				} else {
				callback({ iCode: -1, data: jsonResponse || xhr.responseText });
				}
			};
			xhr.onerror = function () {
				callback({ iCode: -1, data: xhr.statusText });
			};
		...
		sharePointCreateFolderSync: function (url, body, callback) {
			...
			xhr.onload = function () {
				...
				// status 2xx = OK
				if (xhr.status >= 200 && xhr.status < 300) {
					callback({ iCode: 1, data: jsonResponse, status: xhr.status });
				} else {
					// por ejemplo 409 (folder ya existe)
					callback({ iCode: -1, data: jsonResponse || xhr.responseText, status: xhr.status });
				}
			};

			xhr.onerror = function () {
				callback({ iCode: -1, data: xhr.statusText, status: xhr.status });
			};
