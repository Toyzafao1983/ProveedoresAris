...
buildBatchBody: function (aDetalles, method = "MERGE") {
    const batchId = "batch_123";
    const changeSetId = "changeset_abc";
    const useMerge = String(method).toUpperCase() === "MERGE";
    const toNum = v => Number(parseFloat(v || "0").toFixed(3));
    let sBody = "";

    sBody += `--${batchId}\r\n`;
    sBody += `Content-Type: multipart/mixed; boundary=${changeSetId}\r\n\r\n`;

    const aResumenBatch = [];

    // 🔸 Validación previa: no se puede poner precio 0 si hay descuento ≠ 0
    const aBloqueados = aDetalles.filter(o =>
        toNum(o.ConditionRateValue) !== 0 && toNum(o.NetPriceAmount) === 0
    );
    if (aBloqueados.length > 0) {
        const items = aBloqueados.map(i => i.SupplierQuotationItem).join(", ");
        sap.m.MessageBox.warning(
            `Primero debes eliminar el descuento  y guardar antes de establecer el precio en 0.\n\nPosiciones afectadas: ${items}`,
            { title: "Validación comercial" }
        );
        throw new Error("Bloqueo por conflicto entre descuento y precio 0.");
    }

    // 🔸 Procesar posiciones válidas
    aDetalles.forEach(oItem => {
        // === Posición principal ===
        const sItemPath =
            "C_SuplrQuotationItemEnhWD(" +
            "SupplierQuotation='" + oItem.SupplierQuotation + "'," +
            "SupplierQuotationItem='" + oItem.SupplierQuotationItem + "'," +
            "DraftUUID=guid'" + oItem.DraftUUID + "'," +
            "IsActiveEntity=false)";
        const oPayload = {
            NetPriceAmount: String(oItem.NetPriceAmount ?? "0.00"),
            ScheduleLineOrderQuantity: String(oItem.ScheduleLineOrderQuantity ?? "0.00"),
            ScheduleLineDeliveryDate: oItem.ScheduleLineDeliveryDate || null,
            NoteTypeListText: oItem.NoteTypeListText || ""
        };

        sBody += `--${changeSetId}\r\n`;
        sBody += "Content-Type: application/http\r\n";
        sBody += "Content-Transfer-Encoding: binary\r\n\r\n";
        sBody += (useMerge
            ? `POST ${sItemPath} HTTP/1.1\r\nX-HTTP-Method: MERGE\r\n`
            : `PATCH ${sItemPath} HTTP/1.1\r\n`);
        sBody += "Content-Type: application/json; charset=utf-8\r\n";
        sBody += "Accept: application/json\r\n";
        sBody += "If-Match: *\r\n\r\n";
        sBody += JSON.stringify(oPayload) + "\r\n\r\n";

        // === Descuento RA01 ===
        if (oItem.RA01) {
            // RA01 existente → actualizar
            const sCondPath =
                "C_QTNPricingElementWD(" +
                "SupplierQuotation='" + oItem.SupplierQuotation + "'," +
                "SupplierQuotationItem='" + oItem.SupplierQuotationItem + "'," +
                "PricingDocument='" + oItem.RA01.PricingDocument + "'," +
                "PricingDocumentItem='" + oItem.RA01.PricingDocumentItem + "'," +
                "PricingProcedureStep='" + oItem.RA01.PricingProcedureStep + "'," +
                "PricingProcedureCounter='" + oItem.RA01.PricingProcedureCounter + "'," +
                "DraftUUID=guid'" + oItem.RA01.DraftUUID + "'," +
                "IsActiveEntity=false)";
            const oPayloadRA01 = {
                ConditionType: "RA01",
                ConditionRateValue: String(oItem.ConditionRateValue ?? "0"),
            ...
            sBody += "Content-Transfer-Encoding: binary\r\n\r\n";
            sBody += (useMerge
                ? `POST ${sCondPath} HTTP/1.1\r\nX-HTTP-Method: MERGE\r\n`
                : `PATCH ${sCondPath} HTTP/1.1\r\n`);
            sBody += "Content-Type: application/json; charset=utf-8\r\n";
            sBody += "Accept: application/json\r\n";
            sBody += "If-Match: *\r\n\r\n";
            sBody += JSON.stringify(oPayloadRA01) + "\r\n\r\n";

            aResumenBatch.push({
                Item: oItem.SupplierQuotationItem,
                Tipo: "UPDATE RA01",
                ValorAnterior: oItem.RA01.ValorActual,
                ValorNuevo: oItem.ConditionRateValue
            });
        } else {
            // RA01 no existe → crear
            const sCreatePath =
                "C_SuplrQuotationItemEnhWD(" +
                "SupplierQuotation='" + oItem.SupplierQuotation + "'," +
                "SupplierQuotationItem='" + oItem.SupplierQuotationItem + "'," +
                "DraftUUID=guid'" + oItem.DraftUUID + "'," +
                "IsActiveEntity=false)/to_QTNPricingElementWD";
            const oPayloadNewRA01 = {
                ConditionType: "RA01",
                ConditionRateValue: String(oItem.ConditionRateValue ?? "0"),
                ConditionRateValueUnit: "%",
                ConditionAmount: "0.00"
            ...
            sBody += "Content-Type: application/http\r\n";
            sBody += "Content-Transfer-Encoding: binary\r\n\r\n";
            sBody += `POST ${sCreatePath} HTTP/1.1\r\n`;
            sBody += "Content-Type: application/json; charset=utf-8\r\n";
            sBody += "Accept: application/json\r\n\r\n";
            sBody += JSON.stringify(oPayloadNewRA01) + "\r\n\r\n";
        ...
        // === Precio PBXX ===
        if (oItem.PBXX && toNum(oItem.NetPriceAmount) !== toNum(oItem.PBXX.ValorActual)) {
            const sCondPath =
                "C_QTNPricingElementWD(" +
                "SupplierQuotation='" + oItem.SupplierQuotation + "'," +
                "SupplierQuotationItem='" + oItem.SupplierQuotationItem + "'," +
                "PricingDocument='" + oItem.PBXX.PricingDocument + "'," +
                "PricingDocumentItem='" + oItem.PBXX.PricingDocumentItem + "'," +
                "PricingProcedureStep='" + oItem.PBXX.PricingProcedureStep + "'," +
                "PricingProcedureCounter='" + oItem.PBXX.PricingProcedureCounter + "'," +
                "DraftUUID=guid'" + oItem.PBXX.DraftUUID + "'," +
                "IsActiveEntity=false)";
            const oPayloadPBXX = {
                ConditionType: "PBXX",
                ConditionRateValue: String(oItem.NetPriceAmount ?? "0.00"),
                ConditionRateValueUnit: "PEN",
                ConditionAmount: String(oItem.NetPriceAmount ?? "0.00")
            };
            sBody += `--${changeSetId}\r\n`;
            sBody += "Content-Type: application/http\r\n";
            sBody += "Content-Transfer-Encoding: binary\r\n\r\n";
            ...
            sBody += "Accept: application/json\r\n";
            sBody += "If-Match: *\r\n\r\n";
            sBody += JSON.stringify(oPayloadPBXX) + "\r\n\r\n";

            aResumenBatch.push({
                Item: oItem.SupplierQuotationItem,
                Tipo: "UPDATE PBXX",
                ValorAnterior: oItem.PBXX.ValorActual,
                ValorNuevo: oItem.NetPriceAmount
            });
        }
    });
    ...
    console.table(aResumenBatch);
    console.log(`📦 Batch final construido con ${aResumenBatch.length} operaciones RA01/PBXX`);
    return sBody;
}
	};
});